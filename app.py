        ...
        if input_text.strip() == "":
            ...
            with st.spinner('Menganalisis...'):
                try:
                    ...
                    # Detail preprocessing (opsional)
                    if show_details:
                        st.markdown("**🛠️ Teks setelah preprocessing:**")
                        st.write(processed)

                    # Bandingkan model (opsional)
                    if show_comparison:
                        st.markdown("**📌 Perbandingan Model:**")
                        st.write(f"BernoulliNB: {pred_bnb} ({prob_bnb.max()*100:.1f}%)")
                        st.write(f"SVM: {pred_svm} ({prob_svm.max()*100:.1f}%)")
